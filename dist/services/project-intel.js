...
export async function buildProjectIntel(projectId) {
    const [project, statusCounts, severityCounts, recentIssues, recentComments] = await Promise.all([
    ...
    if (recentIssues.length) {
        ...
        for (const issue of recentIssues) {
            ...
            lines.push(`- ${issue.issueKey} [${issue.type}/${issue.status}${sev}${pri}] ${issue.title} (${issue._count.comments} comments)`);
        }
    }
...
// related issues.
export async function buildIssueIntel(issueId) {
    ...
    if (issue.environment)
        lines.push(`- environment: ${issue.environment}`);
    if (labels.length)
    ...
    if (issue.expectedResult || issue.actualResult) {
        ...
        if (issue.expectedResult)
            lines.push(`Expected result: ${issue.expectedResult}`);
        if (issue.actualResult)
            lines.push(`Actual result: ${issue.actualResult}`);
    }
    if (criteria.length) {
        lines.push('');
...
//# sourceMappingURL=project-intel.js.map