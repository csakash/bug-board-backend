...
Return ONLY a JSON object with this exact shape:
{
  ...
  "openQuestions": string[]
    ...
    try {
        ...
        const result = await model.generateContent(imageParts.length ? [prompt, ...imageParts] : prompt);
...
function sanitizeFieldPatch(raw) {
    const out = {};
    ...
    for (const key of ALLOWED_PATCH_KEYS) {
        ...
        if (key === 'stepsToReproduce' || key === 'acceptanceCriteria') {
            ...
            if (ISSUE_TYPES.includes(String(value)))
            ...
            if (value === null)
                out.severity = null;
            else if (SEVERITIES.includes(String(value)))
                out.severity = String(value);
        }
        else if (key === 'priority') {
            if (value === null)
                out.priority = null;
            else if (PRIORITIES.includes(String(value)))
                out.priority = String(value);
        }
        else {
            ...
            if (value === null) {
                ...
                if (key === 'title' && value.trim().length === 0)
                    continue;
                out[key] = value;
            }
        }
...
function normalizeIssueAction(raw) {
    if (!raw || typeof raw !== 'object')
        return null;
    const obj = raw;
...
export async function generateIssueAgentReply(input) {
    if (!genAI) {
        return {
            reply: 'AI is not configured in this environment, so I can’t analyze this issue right now. Set GEMINI_API_KEY to enable the assistant.',
            action: null,
        };
    }
    const model = genAI.getGenerativeModel({ model: env.gemini.model });
...
Return ONLY a JSON object with this exact shape:
{
  ...
  "action": null | {
    ...
    try {
        ...
        return {
            reply: "I had trouble analyzing this issue just now. Could you rephrase, or tell me what you'd like to do?",
            action: null,
        };
    }
}
