...
async function runContextGeneration(projectId: string): Promise<void> {
  try {
    ...
    const fileSummaries = project.files.map(
      (pf) => `${pf.file.fileName} (${pf.file.contentType}, ${pf.purpose})`,
    );
    ...
    if (isR2Configured) {
      const imageFiles = project.files
        .filter(
          (pf) =>
            pf.file.contentType.startsWith('image/') &&
...
projectsRouter.get(
  ...
  asyncHandler(async (req: AuthedRequest, res) => {
    ...
    const data = await remember(`workspace:${workspaceId}:projects`, 10_000, async () => {
      ...
      const activeCountByProjectId = new Map(
        activeCounts.map((entry) => [entry.projectId, entry._count._all]),
      );
