...
export interface IssueInput {
  ...
  source?: 'manual' | 'ai_chat' | 'import' | 'api';
...
function pick<T extends string>(value: unknown, allowed: T[], fallback: T | null): T | null {
  return allowed.includes(value as T) ? (value as T) : fallback;
}

// Create an issue with an atomic per-project issue number/key, labels and files.
export async function createIssue(projectId: string, input: IssueInput) {
  return prisma.$transaction(async (tx) => {
    ...
    const issue = await tx.issue.create({
      data: {
        ...
        issueNumber,
        issueKey,
        type: (pick(input.type, ISSUE_TYPES, 'bug') ?? 'bug') as never,
        title: input.title,
        description: input.description,
        status: (pick(input.status, STATUSES, 'open') ?? 'open') as never,
        severity: pick(input.severity, SEVERITIES, null) as never,
        priority: pick(input.priority, PRIORITIES, null) as never,
